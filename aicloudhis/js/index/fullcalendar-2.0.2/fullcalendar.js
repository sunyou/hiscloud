...
function BasicView(element, calendar, viewName) {
	...
	function buildCellHTML(date) { // date assumed to have stripped time
		...
		/*if (showNumbers) {//月视图天数数字显示 
		    ...
		    var cnMonth = CnMonthofDate(d);//农历月 
		    var cnDay = CnDayofDate(d);//农历日 
		    var solar = SolarTerm(d);//农历节气 
		    
		   // alert("cnmonthcndaysolar"+cnMonth+cnDay+solar);
		    ...
		    if(holiday!=''){
		    	html +="<span class='solarday'>"+holiday+"</span>" + date.date() + "</div>"; 
		    }else{
		    	html +="<span class='solarday'>"+ cnDay+"</span>"+ date.date() + "</div>"; 
		    }
		} */
...
function AgendaView(element, calendar, viewName) {
	...
	function buildSkeleton() {
		...
		slotScroller =
			$("<div style='position:absolute;width:100%;overflow-x:hidden;overflow-y:auto;' class='divmonthcqq'/>")
				.appendTo(slotLayer);
		slotContainer =
