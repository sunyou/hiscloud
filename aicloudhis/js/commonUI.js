...
//	    showFooter: true, 		// 显示表尾
	 ...
	 * 默认皮肤名称
	 */
	this._EASYUI_DEFAULT_THEMES = 'bootstrap';

	/**
	 * 皮肤在cookie里保存的天数
	 */
	this._EASYUI_THEMES_DELAY = 0;

	/**
	 * 皮肤在cookie里变量名
	 */
	this._EASYUI_THEMES_CNAME = '__current_theme';
	...
	this._useIcon = function(theme) {
		theme = theme || this._EASYUI_DEFAULT_THEMES;
		if(jQuery.inArray(theme, ['dhcc-metro']) > -1) {
			includeCSS("/js/easyui/themes/icon-"+theme+".css");
		} else {
			includeCSS("/js/easyui/themes/icon.css");
		}
		includeCSS("/js/easyui/themes/"+theme+"/textbox.css");
	};
	...
	this._init = function(){
		if(!this.isLoadingComplete()){
			...
			$(_bodyElement).addClass('body-mask');
			$(_bodyElement).width('100%').height('100%');
			var theme = getCookie(this._EASYUI_THEMES_CNAME);
			if(!theme) {
				theme = this._EASYUI_DEFAULT_THEMES;
			}
			if(this._EASYUI_THEMES_DELAY > 0) {
				addCookie(this._EASYUI_THEMES_CNAME, 
						theme, this._EASYUI_THEMES_DELAY);
			}
			this._useIcon(theme);
	...
	this.datagridgroup = function(obj, url, queryParams, groupField, groupFormatter, columns, pageOpts, sortOpts, options) {
		...
		if(!obj) return;
		var jqobj = getJqueryDomElement(obj);
		includeJS("/js/easyui/extension/datagrid-groupview.js");
		var _options = jQuery.extend({}, { 
	...
	this.datagridfilter = function(obj, url, queryParams, filterColumns, columns, pageOpts, sortOpts, options) {
		...
		if(!obj) return;
		var jqobj = getJqueryDomElement(obj);
		includeJS("/js/easyui/extension/datagrid-filter.js");
	...
	this.fullcalendar = function(obj, currDate, url, params, method, eventClick, dayClick, options) {
	    ...
	    includeJS('/js/fullcalendar/lang/zh-cn.js');
...
{
	theme = theme || $CommonUI._EASYUI_DEFAULT_THEMES;
	if(theme == easyloader.theme) return;
	var newer = ''+theme,newWrap = '/'+newer+'/';
	var older = ''+easyloader.theme,oldWrap = '/'+older+'/';
	...
	//cookie
	addCookie($CommonUI._EASYUI_THEMES_CNAME, newer, $CommonUI._EASYUI_THEMES_DELAY);
}
