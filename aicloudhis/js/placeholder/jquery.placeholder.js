...
(function ($) {
  var attr = 'placeholder', nativeSupported = attr in document.createElement('input')
	var attrList = "input["+attr+"]"+",textarea["+attr+"]"+",select["+attr+"]";
  $.fn.placeholder = function (options) {
    return $(attrList).not(":hidden").each(function () {
      var $input = $(this)
      ...
      if (opt.hideOnFocus) {
        $input.live('focus' + ns, hide)
      }else{
        $input.live('keypress keydown' + ns, function(e) {
          var key = e.keyCode
      ...
      // ie9 不支持 jq bind 此事件
      if($input.get(0) && 'onpropertychange' in $input.get(0))
