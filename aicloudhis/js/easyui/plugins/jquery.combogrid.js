...
_27.datagrid("load",$.extend({},_26.queryParams,{"filterStr":encodeURI(q)}));
}else{
if(!q){
return;
}
...
$.fn.combogrid.defaults=$.extend({},$.fn.combo.defaults,$.fn.datagrid.defaults,{banOverflow:false,loadMsg:null,idField:null,textField:null,mode:"local",keyHandler:{up:function(){
_14(this,-1);
},down:function(){
_14(this,1);
},enter:function(){
...
if(opts.banOverflow){
var _grid=$.data(this,"combogrid").grid;
var selections=_grid.datagrid("getRows");
var getted=$(this).combogrid("getValues");
var idValues=[], textValues=[];
$.each(selections, function(){
var listItemId=this[opts.idField];
var listItemText=this[opts.textField];
if(undefined!=listItemId)
idValues.push(listItemId);
if(undefined!=listItemText)
textValues.push(listItemText);
});
var realValues=[];
for(var op=0;op<getted.length;op++){
for(var di=0;di<idValues.length;di++){
if(idValues[di]==getted[op]) realValues.push(getted[op]);
}
for(var ti=0;ti<textValues.length;ti++){
if(textValues[ti]==getted[op]) realValues.push(getted[op]);
}
}
getted=realValues;
_1c(this,getted);
